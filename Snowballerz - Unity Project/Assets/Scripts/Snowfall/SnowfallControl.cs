            ...
            if (colliderNum > 2)
            {
                ...
                foreach (int tile in deathTiles)
                {
                    if (insideData.GetCollider(i, 0) == ps.trigger.GetCollider(tile) || insideData.GetCollider(i, 1) == ps.trigger.GetCollider(tile) || insideData.GetCollider(i, 2) == ps.trigger.GetCollider(tile))
                    {
                        ...
                        if (p.startSize < 0.4F)
                        {
                            var gs = findTileObject(tile).GetComponent<GridSquare>();

                            // Don't add a flake if there is an object on the tile.
                            if ( gs.State == GridSquare.GSState.Placeable && !gs.HasCurrentObject() )
                                gs.addFlake();
